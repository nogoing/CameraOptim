...
def data_to_frame(co3d_data, device, args):
    ...
    src_c2ws = c2ws[1:]
    src_idxs = get_nearest_src(tgt_c2w, src_c2ws, args.N_src) + 1
...
def batched_angular_dist_rot_matrix(R1, R2):
    ...
    return np.arccos(np.clip((np.trace(torch.matmul(R2.permute(0, 2, 1), R1), axis1=1, axis2=2) - 1) / 2.,
...
def angular_dist_between_2_vectors(vec1, vec2):
    ...
    angular_dists = np.arccos(np.clip(torch.sum(vec1_unit*vec2_unit, axis=-1), -1.0, 1.0))
